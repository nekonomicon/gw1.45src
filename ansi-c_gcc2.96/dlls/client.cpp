...
// added "void DropPrimaryWeapon( CBasePlayer * player ); to define function
...
int AddToFullPack( struct entity_state_s *state, int e, edict_t *ent, edict_t *host, int hostflags, int player, unsigned char *pSet )
{
	...
	memcpy( &state->origin.x, &ent->v.origin.x, 3 * sizeof( float ) );
	memcpy( &state->angles.x, &ent->v.angles.x, 3 * sizeof( float ) );
	memcpy( &state->mins.x, &ent->v.mins.x, 3 * sizeof( float ) );
	memcpy( &state->maxs.x, &ent->v.maxs.x, 3 * sizeof( float ) );

	memcpy( &state->startpos.x, &ent->v.startpos.x, 3 * sizeof( float ) );
	memcpy( &state->endpos.x, &ent->v.endpos.x, 3 * sizeof( float ) );
	...
	if ( player )
	{
		memcpy( &state->basevelocity.x, &ent->v.basevelocity.x, 3 * sizeof( float ) );
