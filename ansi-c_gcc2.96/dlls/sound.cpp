...
*/
	...
	if ( FBitSet ( pev->spawnflags, AMBIENT_SOUND_NOT_LOOPING ) )
		...
		m_fLooping = TRUE;
	Precache( );
   m_fResetActive = m_fActive;
}

void CAmbientGeneric :: Flush( void )
{
   char* szSoundFile = (char*) STRING(pev->message);

	if (m_fActive != m_fResetActive)
      {
      m_fActive = m_fResetActive;
      if (m_fActive )
	      {
