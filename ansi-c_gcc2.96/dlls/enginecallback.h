...
/*
inline void MESSAGE_BEGIN( int msg_dest, int msg_type, const float *pOrigin
= NULL, edict_t *ed = NULL )
{
        // This just shows you all of your messages and when they were queued
        g_engfuncs.pfnAlertMessage( at_console, "%i, %f message %i\n", msg_dest, gpGlobals->time, msg_type );

        // You could pass in an estimate of the message size and print that, too, but you'd have to change all of your code.

        // Call the actual begin
        (*g_engfuncs.pfnMessageBegin)(msg_dest, msg_type, pOrigin, ed);
}

inline int REG_USER_MSG	(const char *pszName, int iSize)
{
   int x = (*g_engfuncs.pfnRegUserMsg) (pszName, iSize);
   g_engfuncs.pfnAlertMessage( at_console, "%s message is %i\n", pszName, x);
   return x;     
}
*/
