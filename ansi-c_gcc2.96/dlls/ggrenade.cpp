...
void CGrenade::Explode( Vector vecSrc, Vector vecAim )
{
	...
	Explode( &tr, DMG_BLAST | DMG_NEVERGIB);
}

void CGrenade::ExplodeProximity(Vector vecSrc, float flRadius)
{
	...
	while ((pEntity = UTIL_FindEntityInSphere( pEntity, vecSrc, flRadius )) != NULL)
	   {
      if (!FClassnameIs( pEntity->pev, "weaponbox"))
         continue;
      if (pEntity->pev->iuser1 == WEAPON_HEGRENADE)
         {
         SpawnExplode (VARS(pev->owner), pEntity->pev->origin);
         pEntity->SUB_Remove();
         }
      }
}
...
void CGrenade::Explode( TraceResult *pTrace, int bitsDamageType )
{
   ...
   ExplodeProximity(pev->origin, pev->dmg * 2.5);
}
...
void CGrenade::BounceTouch( CBaseEntity *pOther )
{
	...
	if (m_flNextAttack < gpGlobals->time && pev->velocity.Length() > 30)
	{
		...
		if (pevOwner)
		{
			...
			ClearMultiDamage( );
			pOther->TraceAttack(pevOwner, 8 + rand() & 7, gpGlobals->v_forward, &tr, DMG_CLUB );
...
CGrenade * CGrenade:: ShootTimed( entvars_t *pevOwner, Vector vecStart, Vector vecVelocity, float time )
{
	...
	SET_MODEL(ENT(pGrenade->pev), HE_WEAPON_W);
...
CGrenade * CGrenade:: ShootSmokeTimed( entvars_t *pevOwner, Vector vecStart, Vector vecVelocity, float time )
{
	return NULL;
}

extern int gmsgSmokeWindow;
void CGrenade::SmokeWindow( void )
{
	int			i;
   for ( i = 1; i <= gpGlobals->maxClients; i++ )
	   {
		...
		if (pPlayer) 
         {
         MESSAGE_BEGIN(MSG_ONE, gmsgSmokeWindow, NULL, pPlayer->edict());
		      WRITE_COORD( pev->origin.x );
		      WRITE_COORD( pev->origin.y );
		      WRITE_COORD( pev->origin.z );
         MESSAGE_END();
...
CGrenade * CGrenade:: SpawnSmokeExplode( entvars_t *pevOwner, Vector vecStart)
{
	CGrenade *pGrenade = GetClassPtr( (CGrenade *)NULL );
	pGrenade->Spawn();
	UTIL_SetOrigin( pGrenade->pev, vecStart );
...
CGrenade * CGrenade:: SpawnExplode( entvars_t *pevOwner, Vector vecStart)
{
	CGrenade *pGrenade = GetClassPtr( (CGrenade *)NULL );
	pGrenade->Spawn();
	UTIL_SetOrigin( pGrenade->pev, vecStart );
