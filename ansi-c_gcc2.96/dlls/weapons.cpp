...
void W_Precache(void)
{
   ...
   */
   ...
   UTIL_PrecacheOtherWeapon( "weapon_1911" );
	UTIL_PrecacheOtherWeapon( "weapon_b93r" );
	UTIL_PrecacheOtherWeapon( "weapon_knife" );
	UTIL_PrecacheOtherWeapon( "weapon_hegrenade" );

   UTIL_PrecacheOtherWeapon( "weapon_92d" );
   ...
   /*
   FIXME
   if (aw.pistols & 4) UTIL_PrecacheOtherWeapon( "weapon_92d" );
	if (aw.shotguns & 1) UTIL_PrecacheOtherWeapon( "weapon_m3" );
	if (aw.shotguns & 2) UTIL_PrecacheOtherWeapon( "weapon_xm1014" );
   if (aw.subs & 1) UTIL_PrecacheOtherWeapon( "weapon_mac10" );
   if (aw.subs & 2) UTIL_PrecacheOtherWeapon( "weapon_tmp" );
   if (aw.subs & 4) UTIL_PrecacheOtherWeapon( "weapon_mp5navy" );
   if (aw.subs & 8) UTIL_PrecacheOtherWeapon( "weapon_ump45" );
   if (aw.autos & 1) UTIL_PrecacheOtherWeapon( "weapon_ak47" );
   if (aw.autos & 2) UTIL_PrecacheOtherWeapon( "weapon_m16" );
   if (aw.autos & 8) UTIL_PrecacheOtherWeapon( "weapon_sg552" );
   if (aw.autos & 16) UTIL_PrecacheOtherWeapon( "weapon_sg550" );
   */
...
void CBasePlayerItem::DefaultTouch( CBaseEntity *pOther )
{
	...
	if (pOther->AddPlayerItem( this ))
	   {
      //pPlayer->m_pLastItem = pPlayer->m_pActiveItem;
...
void CBasePlayerWeapon::ItemPostFrame( void )
{
   ...
   if (!(m_pPlayer->pev->button & IN_ATTACK))
	...
	if ((m_pPlayer->pev->button & IN_ATTACK2) && CanAttack( m_flNextSecondaryAttack, gpGlobals->time, UseDecrement() ) )
      ...
      if (!(m_pPlayer->m_iTagged & TAG_DONTSHOOT))
		   SecondaryAttack();
...
void BuyDropWeapon(CBasePlayer *pPlayer, int primary);
int CBasePlayerWeapon::AddToPlayer( CBasePlayer *pPlayer )
{
   ...
   if (p.iWeight == WEIGHT_PRIMARY)
      BuyDropWeapon(pPlayer, 1);
   else if (p.iWeight == WEIGHT_SECONDARY)
      BuyDropWeapon(pPlayer, 0);
...
int CBasePlayerWeapon::UpdateClientData( CBasePlayer *pPlayer )
{
   ...
   //pPlayer->m_pActiveItem = pPlayer->m_pClientActiveItem;
...
{
   if (ItemInfoArray[m_iId].iWeight == WEIGHT_PROJECTILE)
...
BOOL CBasePlayerWeapon :: DefaultReload( int iClipSize, int iAnim, float fDelay )
{
   ...
   m_pPlayer->SetAnimation(PLAYER_RELOAD);
	SendWeaponAnim( iAnim, 0 /*UseDecrement() ? 1 : 0*/);
...
void CWeaponBox::Touch( CBaseEntity *pOther )
{
   ...
   if (m_iWeight >= WEIGHT_SECONDARY)
      if (GetWeaponByWeight(pPlayer, m_iWeight))
         return;
	...
	for ( i = 0 ; i < MAX_AMMO_SLOTS ; i++ )
	   {
		if ( !FStringNull( m_rgiszAmmo[ i ] ) )
		   {
			// Give ammo of gun, 1 for grenades and no-ammo guns: 
