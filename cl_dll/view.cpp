...
float V_CalcBob ( struct ref_params_s *pparams )
{
	...
	static float	bob;
	static float	sinewave = 0.5;
	static float	lasttime;
   static float   cycle_inc = .04f;
   static float   last_onground = 0;
	vec3_t	vel;
   ...
   last_onground = 0;
	...
	cycle = bobtime - (int)( bobtime / .5f ) * .5f;
	//cycle /= cl_bobcycle->value;
   cycle /= .5f;
   cycle = M_PI + M_PI * ( cycle - .1 )/( 1.0 - .1f ); //cl_bobup->value
   ...
   // bob is proportional to simulated velocity in the xy plane
	...
	bob = sqrt( vel[0] * vel[0] + vel[1] * vel[1] ) * .14f/*cl_bob->value*/;
	bob = bob * 0.05 + bob * 0.03 * sin(cycle);
	//bob = min( bob, 4 );
	//bob = max( bob, -7 );
	return bob;
...
void V_CalcViewRoll ( struct ref_params_s *pparams )
{
   ...
   myviewangle = pparams->viewangles;
...
void V_Init (void)
{
   ...
   cl_teamscores     = gEngfuncs.pfnRegisterVariable( "cl_teamscores","0", 0 );
   cl_3dradar        = gEngfuncs.pfnRegisterVariable( "cl_3dradar","1", 0 );
   cl_2dradar        = gEngfuncs.pfnRegisterVariable( "cl_2dradar","1", 0 );
   cl_beats          = gEngfuncs.pfnRegisterVariable( "cl_beats","1", 0 );
   cl_chat           = gEngfuncs.pfnRegisterVariable( "cl_chat","1", 0 );
}
