...
int CHudSayText :: Draw( float flTime )
{
	...
	for ( int i = 0; i < MAX_LINES; i++ )
	{
		if ( *g_szLineBuffer[i] )
		{
			if ( /*1 || *g_szLineBuffer[i] == 2 &&*/ g_pflNameColors[i] )
			{
            ...
            //if (g_pflNameColors[i])
               //gEngfuncs.pfnDrawSetTextColor( g_pflNameColors[i][0], g_pflNameColors[i][1], g_pflNameColors[i][2] );
				//gEngfuncs.pfnDrawSetTextColor(0xFF, 0, 0);
...
int CHudSayText :: MsgFunc_SayText( const char *pszName, int iSize, void *pbuf )
{
   ...
   //char x[245];
	...
	int client_team = READ_BYTE();
	int client_beep = READ_BYTE();
   //sprintf (x, "%i - %s", client_team, READ_STRING());
   //SayTextPrint( x, iSize - 1,  client_index, client_team );
	SayTextPrint( READ_STRING(), iSize - 1,  client_index, client_team, client_beep);
	
	return 1;
}

float *GetTeamColor( int teamIndex );
void CHudSayText :: SayTextPrint( const char *pszBuf, int iBufSize, int clientIndex, int client_team, int client_beep)
{
	if ( gViewPort && gViewPort->AllowedToPrintText() == FALSE )
		return;

   if (!cl_chat->value) return;
			...
			if ( nameInString )
			   {
				...
				g_pflNameColors[i] = GetTeamColor( client_team );
   			}
		   }
	   }
   */
   //if (clientIndex > -1)
      g_pflNameColors[i] = GetTeamColor( client_team );
   //else
      //g_pflNameColors[i] = GetTeamColor(0);
   ...
   if (client_beep)
      ...
      switch (client_team)
         {
         case 1:
   	      PlaySound( "common/menu1.wav", 1 );
            break;
         case 2:
	         PlaySound( "common/menu2.wav", 1 );
            break;
         }
      }
